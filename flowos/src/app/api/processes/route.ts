...
export async function GET(req: NextRequest) {
  ...
  const flat = searchParams.get("flat"); // flat=1 → todos los procesos de la org (sin carpetas, lista liviana)

  try {
    // Modo flat: lista plana de procesos para selectores (ej. nodo "Llamar proceso").
    // Ignora el árbol de carpetas y excluye las carpetas en sí.
    if (flat) {
      const data = await db
        .select({
        ...
        .where(and(eq(processDefinitions.organizationId, orgId), ne(processDefinitions.category, "folder")))
